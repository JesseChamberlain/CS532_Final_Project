...
def plot_model_comparison():
    ...
    pivot_time = df.pivot(index="Model", columns="Scenario", values="Train+Eval Time (s)")
...
def plot_partition_benchmark():
    ...
    pivot = df.pivot(index="repartition", columns="shuffle_partitions", values="runtime_seconds")
    ...
    bars = axes[1].barh(df_sorted["config"], df_sorted["runtime_seconds"], color=colors, edgecolor="black")
...
def plot_cache_benchmark():
    ...
    bars1 = ax.bar(x - width/2, df["first_pass_seconds"], width, label="First Pass (Cold)", color="#ff7f0e", edgecolor="black")
    bars2 = ax.bar(x + width/2, df["second_pass_seconds"], width, label="Second Pass (Cache Hit)", color="#2ca02c", edgecolor="black")
    ...
    for bar in bars1:
        ...
        ax.annotate(f"{height:.3f}s", xy=(bar.get_x() + bar.get_width()/2, height), xytext=(0, 3), textcoords="offset points", ha="center", fontsize=9)
    for bar in bars2:
        height = bar.get_height()
        ax.annotate(f"{height:.3f}s", xy=(bar.get_x() + bar.get_width()/2, height), xytext=(0, 3), textcoords="offset points", ha="center", fontsize=9)
    ...
    for i, row in df.iterrows():
        ...
        ax.annotate(f"{speedup:.1f}x faster", xy=(i, max(row["first_pass_seconds"], row["second_pass_seconds"]) + 0.05), ha="center", fontsize=9, color="blue", fontweight="bold")
...
def plot_fault_tolerance():
    ...
    times = [row["first_time_seconds"], row["second_time_seconds"], row["third_time_seconds"]]
    ...
    for bar, time in zip(bars, times):
        height = bar.get_height()
        ax.annotate(f"{time:.3f}s", xy=(bar.get_x() + bar.get_width()/2, height),  xytext=(0, 5), textcoords="offset points", ha="center", fontsize=12, fontweight="bold")

    # Add explanatory annotations
    cache_speedup = row["first_time_seconds"] / row["second_time_seconds"]
    ax.annotate(f"Cache provides\n{cache_speedup:.1f}x speedup", xy=(1, row["second_time_seconds"]), xytext=(1.5, 0.12), arrowprops=dict(arrowstyle="->", color="green"), fontsize=10, color="green")

    ax.annotate("Lineage enables\nfault recovery", xy=(2, row["third_time_seconds"]), xytext=(2.3, 0.08), arrowprops=dict(arrowstyle="->", color="red"), fontsize=10, color="red")
...
def plot_scalability():
    ...
    ax1 = axes[0]
    bars = ax1.bar(df["cores"].astype(str), df["throughput_rows_per_sec"],
    ...
    ax2 = axes[1]
    ax2.plot(df["cores"], df["speedup_vs_1_core"], "o-", markersize=12, linewidth=3, color="#2ca02c", label="Actual Speedup")
    ax2.plot(df["cores"], df["cores"], "--", linewidth=2, color="gray", label="Ideal Linear Scaling")
    ...
    for i, row in df.iterrows():
        ...
        ax2.annotate(f"{efficiency:.0f}% efficient",
...
def plot_class_imbalance():
    ...
    for bar, size in zip(bars, sizes):
        axes[1].annotate(f"{size:,}", xy=(bar.get_x() + bar.get_width()/2, bar.get_height()),
